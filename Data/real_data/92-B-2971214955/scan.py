...
if __name__=="__main__":
    ...
    fixed_params = fixed_params | {
        "rel_amps":jnp.array([3.66,0.5]),
        "means": jnp.array([0.1,-0.9,-0.4])
    }

    #gridloss = lambda t1,t2,ltau: loss(data, fixed_params | {"lags": jnp.array([t1,t2])})
    gridloss = lambda t1,t2,ltau,ramp2: loss(data, fixed_params | {"lags": jnp.array([t1,t2]), "log_sigma_c":0.12 + (ltau-5.54)*0.5, "log_tau": ltau, "rel_amps":jnp.array([3.66, ramp2])})
    ...
    log_taus = np.linspace(4.5, 7, nint)
    ramp2s   = np.linspace(0, 2, nint)
